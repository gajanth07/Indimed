...
DRUGS={'paracetamol': {'dose_mgkg':15,'route':'PO/IV','frequency':'every 4-6 hours','max':'Max 60-75 mg/kg/day; do not exceed product/protocol maximum.'},'ibuprofen': {'dose_mgkg':10,'route':'PO','frequency':'every 6-8 hours','max':'Max 30-40 mg/kg/day depending on protocol.'},'ondansetron': {'dose_mgkg':0.15,'route':'PO/IV','frequency':'every 8 hours as needed','max':'Maximum 8 mg per dose in many pediatric protocols.'},'amoxicillin': {'dose_mgkg':45,'route':'PO','frequency':'per day in 2 divided doses','max':'Higher total daily doses may be used for selected infections per protocol.'},'linezolid': {'dose_mgkg':10,'route':'PO/IV','frequency':'every 8-12 hours depending on age/protocol','max':'Maximum 600 mg per dose.'},'cefixime': {'dose_mgkg':8,'route':'PO','frequency':'per day once daily or in 2 divided doses','max':'Use product-specific maximum.'},'ceftriaxone': {'dose_mgkg':50,'route':'IV/IM','frequency':'every 24 hours','max':'Up to 100 mg/kg/day in selected severe infections per protocol.'},'cefotaxime': {'dose_mgkg':50,'route':'IV','frequency':'every 6-8 hours','max':'Severe infection protocols may use higher total daily doses.'},'cephalexin': {'dose_mgkg':25,'route':'PO','frequency':'per dose every 6-8 hours','max':'Use infection-specific maximum daily dose.'},'azithromycin': {'dose_mgkg':10,'route':'PO','frequency':'every 24 hours','max':'Often day-1 regimen based; follow indication-specific protocol.'},'clindamycin': {'dose_mgkg':10,'route':'PO/IV','frequency':'every 6-8 hours','max':'Follow indication-specific maximum.'},'metronidazole': {'dose_mgkg':7.5,'route':'PO/IV','frequency':'every 8 hours','max':'Use infection-specific maximum.'},'prednisolone': {'dose_mgkg':1,'route':'PO','frequency':'daily or divided per indication','max':'Protocol-specific ceiling applies.'},'dexamethasone': {'dose_mgkg':0.15,'route':'PO/IV/IM','frequency':'single dose or scheduled per indication','max':'Protocol-specific ceiling applies.'},'salbutamol': {'dose_text':'Nebulized or inhaled salbutamol is device- and age-based; use local inhalation protocol.','route':'Neb/Inhaled'},'budesonide': {'dose_text':'Nebulized/inhaled budesonide depends on formulation and severity; use product-specific pediatric protocol.','route':'Neb/Inhaled'},'cetirizine': {'dose_text':'Age-banded pediatric dosing is preferred over mg/kg; use product-specific age schedule.','route':'PO'},'zinc': {'dose_text':'Common pediatric diarrhea regimen: 10 mg daily if under 6 months; 20 mg daily if 6 months or older.','route':'PO'},'ors': {'dose_text':'ORS is rehydration-plan based, not a mg/kg drug dose; use dehydration tool for fluid volume.','route':'PO'}}
    ...
    if dept=='AI Clinical Search':
        ...
        with tabs[0]:
            with st.form('m1'):
                ...
                wt=c2.number_input('Patient weight (kg)',1.0,200.0,20.0)
                s=st.form_submit_button('Calculate personalized dose')
            if s:
                source_badges(indian='General pediatric dose support', global_src='Weight-based dosing references', method='Simple dose calculation')
                entry = pediatric_dose_entry(drug)
                if entry and 'dose_mgkg' in entry:
                    dose_mg = entry['dose_mgkg']*wt
                    c1,c2=st.columns(2)
                    c1.metric('Personalized dose', f"{dose_mg:.2f} mg")
                    c2.metric('Dose basis', f"{entry['dose_mgkg']:.2f} mg/kg from patient weight")
                    extras=[]
                    if entry.get('frequency'): extras.append(f"Frequency: {entry['frequency']}")
                    if entry.get('route'): extras.append(f"Route: {entry['route']}")
                    if entry.get('max'): extras.append(f"Note: {entry['max']}")
                    if extras:
                        st.markdown(f"<div class='box'><b>General dosing details</b><br>Patient weight used: {wt:.1f} kg<br>{'<br>'.join(extras)}</div>", unsafe_allow_html=True)
                elif entry and 'dose_text' in entry:
                    st.markdown(f"<div class='box'><b>General dosing details</b><br>{entry['dose_text']}<br>Route: {entry.get('route','Varies')}</div>", unsafe_allow_html=True)
                else:
                    st.error('No embedded general dose entry found for this drug.')
                safety_note('This dose calculator now gives a general dose output only. Final prescription still requires formulation, frequency, age-band, and indication review.','note')
        with tabs[1]:
            with st.form('m2'):
                c1,c2=st.columns(2)
